...
class MCOWL():
    ...
    def __init__(self, act="relu", layer=5, width=32, loss="lsl"):
    ...
    def fit(self, Y, X, A, Yt, Xt, At, optAt, epochs=100, learning_rate=1e-3, verbose=0, opt_func=Adam, weight_decay=0.01, 
        ...
        if self.loss=='ghl':
            ...
            print('random loss:',self.model.generalized_hinge_loss(torch.from_numpy(np.random.choice([-1,1],(Xt.shape[0], output_dim))).float(), At_tsr, Yt_tsr))
        elif self.loss=='hml':
            np.random.seed()
            print('opt loss:', self.model.hamming_loss(optAt_tsr, At_tsr, Yt_tsr))
            print('random loss:',self.model.hamming_loss(torch.from_numpy(np.random.choice([-1,1],(Xt.shape[0], output_dim))).float(), At_tsr, Yt_tsr))
        elif self.loss=="lsl":
            np.random.seed()
            print('opt loss:', self.model.landslide_loss(optAt_tsr, At_tsr, Yt_tsr))
            print('random loss:',self.model.landslide_loss(torch.from_numpy(np.random.choice([-1,1],(Xt.shape[0], output_dim))).float(), At_tsr, Yt_tsr))
